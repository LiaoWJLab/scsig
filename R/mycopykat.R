...
#'
#' @return  aneuploid/diploid prediction results; 2.CNA results in 220KB windows; 3.heatmap; 4.hclustering object.
#' @examples
#' @export
mycopykat <- function(rawmat,
                      id.type        ="S",
                      cell.line      ="no",
                      ngene.chr      =5,
                      LOW.DR         =0.05,
...
if(cell.line=="yes"){
  ...
  saveRDS(hcc, file = paste(save_path,"2-",sample.name,"clustering_results.rds",sep=""))
...
#library(parallelDist)
  ...
  jpeg(paste(save_path,"3-",sample.name,"heatmap.jpeg",sep=""), height=h*250, width=4000, res=100)
   heatmap.3(t(mat.adj),dendrogram="r", distfun = function(x) parallelDist::parDist(x,threads =n.cores, method = distance), hclustfun = function(x) hclust(x, method="ward.D"),
            ...
            symm=F,symkey=F,symbreaks=T,cex=1, main=paste(WNS1,"; ",WNS, sep=""), cex.main=8, margins=c(10,10))
  ...
  saveRDS(hcc, file = paste(save_path,"4-",sample.name,"clustering_results.rds",sep=""))
